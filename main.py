...
# MADE BY KAIF TARASAGAR
    ...
    def update_tree_styles(self):
        # Configure Treeview tags once to avoid high-frequency overhead in refresh loops
        if self.dark_mode:
            ...
            # Dark Mode 'done': Foreground #a9a9a9, background #212529 (~6.4:1 contrast ratio)
            self.tree.tag_configure("done", foreground="#a9a9a9", background="#212529")
        else:
            self.tree.tag_configure("high", background="#e45252")
            self.tree.tag_configure("medium", background="#ebc139")
            self.tree.tag_configure("low", background="#23e24f")
            # Light Mode 'done': Foreground #595959, background #f8f9fa (~6.6:1 contrast ratio)
    ...
    def refresh_list(self):# MADE BY KAIF TARASAGAR
        ...
        for task in self.tasks:
            if search_text in task["task"].lower() or search_text in task["category"].lower():
                ...
                if task["status"] == "Done":
                    tags.insert(0, "done") # 'done' tag prepended to ensure visual precedence
