...
# MADE BY KAIF TARASAGAR
        self.add_btn = tk.Button(self.btn_frame, text="➕ Add", width=14, command=self.add_task, bg="#0d6efd", fg="white")
        self.add_btn.grid(row=0, column=0, padx=6)
        self.update_btn = tk.Button(self.btn_frame, text="✏️ Update", width=14, command=self.update_task, bg="#ffc107", fg="black")
        self.update_btn.grid(row=0, column=1, padx=6)
        self.done_btn = tk.Button(self.btn_frame, text="✅ Mark Done", width=14, command=self.mark_done, bg="#198754", fg="white")
        self.done_btn.grid(row=0, column=2, padx=6)
        self.delete_btn = tk.Button(self.btn_frame, text="🗑️ Delete", width=14, command=self.delete_task, bg="#dc3545", fg="white")
        self.delete_btn.grid(row=0, column=3, padx=6)
        self.save_btn = tk.Button(self.btn_frame, text="💾 Save ", width=14, command=self.save_as_txt, bg="#6f42c1", fg="white")
        self.save_btn.grid(row=0, column=4, padx=6)
        self.toggle_btn = tk.Button(self.btn_frame, text="🌙 Toggle Dark Mode", width=18, command=self.toggle_dark_mode, bg="#343a40", fg="white")
        self.toggle_btn.grid(row=0, column=5, padx=6)
# MADE BY KAIF TARASAGAR
    ...
    def refresh_list(self):# MADE BY KAIF TARASAGAR
        ...
        for task in self.tasks:
            if search_text in task["task"].lower() or search_text in task["category"].lower():
                ...
                tags_list = [priority_tag]
                if task["status"] == "Done":
                    tags_list.append("done")
                self.tree.insert("", tk.END, values=(task["task"], task["category"], task["priority"], task["status"]), tags=tuple(tags_list))
...
# MADE BY KAIF TARASAGAR
    ...
    def toggle_dark_mode(self):
        ...
        if self.dark_mode:
            ...
            self.search_entry.config(bg="#343a40", fg="white", insertbackground="white")
            ...
            self.search_label.config(bg="#f8f9fa", fg="#212529")
